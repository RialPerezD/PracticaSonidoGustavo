...
bool LinkExists(int startAttr, int endAttr) {
    for (const auto& l : links) {
        if (l.startAttr == startAttr && l.endAttr == endAttr) return true;
    }
    return false;
}
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "ImNodes + Audio Graph", nullptr, nullptr);
    if (!window) {
        glfwTerminate();
        return -1;
    }
    ...
    if (!audioManager.Init()) {
        std::cerr << "Failed to init AudioManager\n";
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (currentPlayingNodeId != -1) {
            ...
            if (currentNode) {
                if (!audioManager.IsPlaying(currentNode->audioIndex)) {
                    ...
                    for (const auto& l : links) {
                        if (l.startAttr == currentNode->outputPin) {
                    ...
                    if (nextNodeId != -1) {
                        ...
                        if (nextNode) {
                            audioManager.Play(nextNode->audioIndex, false);
        ...
        if (ImGui::BeginMainMenuBar()) {
            if (ImGui::BeginMenu("Nodos")) {
                if (ImGui::MenuItem("Intro")) {
                    create_audio_node("Intro", "../assets/0_intro.wav");
                }
                if (ImGui::MenuItem("First Piano")) {
                    create_audio_node("First Piano", "../assets/1_firstPiano.wav");
                }
                if (ImGui::MenuItem("Ding ding")) {
                    create_audio_node("Ding ding", "../assets/2_dingDing.wav");
                }
                if (ImGui::MenuItem("This is crazy frog")) {
                    create_audio_node("This is crazy frog", "../assets/3_thisIsCrazyFrog.wav");
                }
                if (ImGui::MenuItem("Body song")) {
                    create_audio_node("Body song", "../assets/4_mainSong.wav");
                }
                if (ImGui::MenuItem("Motillo")) {
                    create_audio_node("Motillo", "../assets/5_motillo.wav");
                }
                ImGui::EndMenu();
            }
        ...
        for (auto& n : audioNodes) {
            ...
            if (ImGui::Button(playId.c_str())) {
                if (currentPlayingNodeId != -1) {
                    ...
                    if (prevNode) audioManager.Stop(prevNode->audioIndex);
                }
        ...
        if (ImNodes::IsLinkCreated(&startAttr, &endAttr)) {
            if (startAttr != endAttr && !LinkExists(startAttr, endAttr)) {
        ...
        if (ImNodes::IsLinkDestroyed(&destroyedLinkId)) {
            links.erase(std::remove_if(links.begin(), links.end(),
                [destroyedLinkId](const Link& link) {
        ...
        links.erase(std::remove_if(links.begin(), links.end(),
            [](const Link& l) {
                for (const auto& n : audioNodes) {
                    if (n.to_delete && (l.startAttr == n.inputPin || l.startAttr == n.outputPin || l.endAttr == n.inputPin || l.endAttr == n.outputPin)) {
                        return true;
                    }
                }
                return false;
        ...
        audioNodes.erase(std::remove_if(audioNodes.begin(), audioNodes.end(),
            [](const AudioNode& n) { return n.to_delete; }),
            audioNodes.end());
